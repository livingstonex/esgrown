...
const NewExercises = ({ service, refreshContentLog, title }) => {
    ...
    const exUrl = `http://18.188.101.36/excercise/add`;
    ...
    const createQuestion = () => {
        ...
        axios.post(`http://18.188.101.36/question/add`, questionData)
            .then(res => {
                ...
                axios.post(`http://18.188.101.36/question/${res.data.excercise_id}`)
    ...
    function getAllJobs(corpid) {
        try {
            axios.get(`http://18.188.101.36/jobs/${corpid}`)
                .then(jobs => {
