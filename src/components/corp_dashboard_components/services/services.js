...
import LMServiceContent from '../../ind_dashboard_components/services-components/service-cards/lmservice';
import RMServiceContent from '../../ind_dashboard_components/services-components/service-cards/rmservice';
...
const CorpServices = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:5000/corporatesubscriptions/${com.id}`)
            .then(res => {
                if (res.data) {
                    const rm = res.data.filter(d => {
                        return d.service === "RM"
                    });
                    const lm = res.data.filter(m => {
                        return m.service === "LM"
                    })
                    setLMsrv(lm[0]);
                    setRMsrv(rm[0]);
    ...
    return (
        <>
            ...
            </>: (page === 1) ? <RMServiceContent /> : (page === 2) ? <LMServiceContent /> : ""}
