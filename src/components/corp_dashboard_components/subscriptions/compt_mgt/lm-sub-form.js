...
const LMSubForm = ({ show, onHide, closeModal }) => {
    ...
    useEffect(() => {
        ...
        axios.get(`http://172.31.25.5:5000/individuals/staff/${userr.id}`)
            .then(res => {
    ...
    const onSuccess = (res) => {
        ...
        staff.map(st => {
            axios.post(`http://172.31.25.5:5000/individuals/update/substatus/${st._id}`, data)
                .then(res => console.log(res.data))
                .catch(err => console.log(err));

            axios.post(`http://172.31.25.5:5000/competence/management/add`, {
                user_id: st._id,
                company_id: user.id,
