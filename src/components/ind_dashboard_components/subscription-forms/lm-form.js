...
export default function Form(props) {
    ...
    const onSubmit = (e) => {
        ...
        try {
            setSpinner(true);
            axios.post('http://13.59.192.18/api/subscriptionlm/add', SubObject)
                .then((res) => {
    ...
    const onSuccess = (res) => {
        ...
        axios.get(`https://api.paystack.co/subscription`, { headers: { "Authorization": "Bearer sk_test_19f4c12e4e018a9f742e1723d42c9c8e509800b4" } })
            .then(res => {
                ...
                axios.post(`http://13.59.192.18/api/subscriptionlm/update/lmref/${userId}`, data)
                    .then(res => console.log(res))
                    .catch(err => console.log(err))


                //update user details
                axios.post(`http://13.59.192.18/api/individuals/update/substatus/${userId}`, { sub_status_lm: client[0].status, })
                    .then(res => {
