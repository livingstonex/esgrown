...
const JobsModal = ({ show, onHide, closeModal }) => {
    ...
    useEffect(() => {
        ...
        axios.get(`http://13.59.192.18/api/jobs`)
            .then(res => {
        ...
        axios.get(`http://13.59.192.18/api/applications/check/${user.id}`)
            .then(res => {
    ...
    const postToServer = () => {
        ...
        try {
            setLoading(true)
            axios.post(`http://13.59.192.18/api/applications/add`, postData)
                .then(res => {
                    setLoading(false);
