...
const JobsModal = ({ show, onHide, closeModal }) => {
    ...
    useEffect(() => {
        ...
        axios.get(`http://172.31.25.5:5000/jobs`)
            .then(res => {
        ...
        axios.get(`http://172.31.25.5:5000/applications/check/${user.id}`)
            .then(res => {
    ...
    const postToServer = () => {
        ...
        try {
            setLoading(true)
            axios.post(`http://172.31.25.5:5000/applications/add`, postData)
                .then(res => {
                    setLoading(false);
